...
export const getRisingSign = (birthDate, birthTime, latitude, longitude) => {
  try {
    ...
    const localSiderealTime = gast + (longitude / 15.0);
    
    // Calculate ascendant from local sidereal time and latitude
    // This is simplified - more accurate would use full house calculations
    const ascendantRA = (localSiderealTime * 15) % 360;
    
    // Convert RA to ecliptic longitude (approximate)
    const obliquity = 23.44; // Earth's obliquity in degrees
    const sinObliq = Math.sin(obliquity * Math.PI / 180);
    const cosObliq = Math.cos(obliquity * Math.PI / 180);
    
    const raRad = ascendantRA * Math.PI / 180;
    const latRad = latitude * Math.PI / 180;
    
    const tanAsc = Math.sin(raRad) * cosObliq + Math.tan(latRad) * sinObliq;
    const ascendantLon = Math.atan2(tanAsc, Math.cos(raRad)) * 180 / Math.PI;
    
    return getZodiacFromLongitude(ascendantLon);
  } catch (error) {
