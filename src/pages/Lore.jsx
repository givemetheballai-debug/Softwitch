      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {LORE_SECTIONS.map((section, index) => (
          ...
          >
            <Link
              to={section.path}
              className="block h-full min-h-[180px] bg-gradient-to-br from-sw-purple/20 to-sw-pink/20 rounded-2xl p-5 hover:from-sw-purple/30 hover:to-sw-pink/30 transition-all border-2 border-sw-teal/30 hover:border-sw-teal hover:scale-105 flex flex-col"
            >
              <div className="text-4xl mb-3">
                {section.emoji}
              </div>
              <h3 className="font-serif text-lg text-white mb-2 flex-shrink-0">
                {section.title}
              </h3>
              <p className="text-sm text-sw-cream/80 flex-grow">
                {section.description}
              </p>
            </Link>
          </motion.div>
