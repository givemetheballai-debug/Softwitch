  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-16">
      ...
      >
        {slides.map((slide, index) => {
          ...
          return (
            ...
            >
              {/* Cosmic Slide - Links to /lore */}
              {slide.type === 'cosmic' && (
                <Link to="/lore" className="block w-full h-full">
                  <div
              ...
              {/* Big Scream Slide - Links to bigscreamenergy.com */}
              ...
              {/* MASH Slide - Links to playcosmicmash.com */}
              {slide.type === 'mash' && (
                <a
      ...
      <motion.div
        ...
        className="relative overflow-hidden bg-gradient-to-br from-sw-purple/10 to-sw-pink/10 rounded-xl p-4 text-center shadow-soft"
        style={{
      ...
      >
        <div className="relative z-10 bg-white/90 backdrop-blur-sm rounded-lg p-8">
          <h2 className="cosmic-snapshot-title font-serif text-sw-purple mb-3">
            🌘 Today's Cosmic Snapshot
          </h2>
          <p className="cosmic-snapshot-date text-sw-purple mb-3">
            {dateStr}
          </p>
          <p className="cosmic-snapshot-details text-sw-purple/90 mb-4">
            <span className="block mb-2">Today's moon: <strong>{moonPhase.name}</strong> — {moonPhase.description}</span>
            <span className="block">Today's season: <strong>{season.name}</strong> — {season.description}</span>
          </p>
          <Link 
            to="/lore/moon-phases"
            className="inline-block mt-2 text-base text-sw-pink hover:text-sw-purple transition-colors font-semibold"
          >
            More on moon phases ✨
          </Link>
        </div>
